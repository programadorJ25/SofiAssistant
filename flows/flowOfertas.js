...
const { getTicket, getUser } = require("../Api/clients.service");
...
const getPrompt = async () => {
  ...
  const text = readFileSync(join(pathPromp, "Ventas.txt"), "utf-8");
  return text;
};
    ...
    return addKeyword("OFERTAS", {
      ...
      .addAction(async (ctx, { endFlow, flowDynamic, provider }) => {
        ...
        const listItems = items.data
          .map(
            (i) =>
              `ARTICULO:${i.attributes.name}, PRECIO_SUGERIDO:${i.attributes.price}`
          )
          .join("\n");
